...
const { todoInit } = require('./todoModel')
  ...
  constructor() {
    ...
    this.subscribe(todos => {
      if(todos.length && !gen){
        ...
        // log(this.base)
        this.flush().render().cluster(todoInit.bind(this))
      } 
      else if(!todos.length && gen){
        gen = false
        this.base.todoapp.template = tmpl(gen)
        this.flush().render()
      }
...
const tmpl = todoLength => {
  ...
  `
  return todoLength ?  staticTmpl.concat(main, footer) : staticTmpl
}
